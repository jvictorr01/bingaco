...
export async function verificarGanhador(drawId: string) {
  ...
  await db.runTransaction(async (transaction) => {
    ...
    const elegiveis = allCards; // todas participam de todas as fases
    ...
    // Adiciona detalhes do ganhador
    if (!drawData.winnerDetails) drawData.winnerDetails = {}

    if (!drawData.winnerDetails[fase]) drawData.winnerDetails[fase] = []

    drawData.winnerDetails[fase].push({
    ...
    updateData.winnerDetails = drawData.winnerDetails
